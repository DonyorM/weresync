...
class GrubPlugin(IBootPlugin):
    ...
    def get_help(self):
        return "Installs the Grub2 bootloader. This works on both UEFI and MBR systems."
    ...
    def install_bootloader(self, source_mnt, target_mnt, copier, excluded_partitions=[],
        ...
        if root_partition == None:
                for i in copier.target.get_partitions():
                    ...
                    raise weresync.exception.CopyError("Could not find partition with 'boot/grub' folder on device {0}".format(copier.target.device))
        ...
        try:
            ...
            if boot_partition != None:
                copier.target.mount_partition(boot_partition, mount_loc + "boot")
                boot_mounted_here = True

            if efi_partition != None:
                os.makedirs(mount_loc + "boot/efi", exist_ok=True)
                copier.target.mount_partition(efi_partition, mount_loc + "boot/efi")
            ...
            if grub_install.returncode != 0:
                raise weresync.exception.DeviceError(copier.target.device, "Error installing grub.", str(install_output, "utf-8"))
