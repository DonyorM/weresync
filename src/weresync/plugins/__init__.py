...
"""This package contains code for bootloader plugins.
 ...
 Bootloader plugin is any class extending :py:class:`~weresync.plugins.IBootPlugin` inside a file name following the regex pattern "^weresync_.*\.py$". These plugins can be installed to the site-packages directory (for example, using pip) or to /usr/local/weresync/plugins.
...
class IBootPlugin(IPlugin):
    ...
    The name of a plugin should simply be its filename, without the "weresync_" prefix or a suffix. So "weresync_grub2.py"'s name would be "grub2".

    :param prettyName: a human readable name for display. Can contain any character.
    :param filename: A unique identifying name that should be easy to type on a terminal. Used by users to tell WereSync which plugin to use. See above for exact definition."""
    ...
    def install_bootloader(self, source_mnt, target_mnt, copier, excluded_partitions=[],
                           boot_partition=None, root_partition=None, efi_partition=None):
...
dirs = ["/usr/local/weresync/plugins", get_python_lib(), path.dirname(__file__)]

regex_analyzer = PluginFileAnalyzerMathingRegex("regex", "^weresync_.*\.py$")
locator = PluginFileLocator([regex_analyzer])

__manager = PluginManager(categories_filter={"bootloader" : IBootPlugin}, directories_list=dirs, plugin_locator=locator)
